...
function Comment({ comment, viewer, onReply, children, depth = 0, onRefresh }) {
  ...
  return (
    <article className={`comment-card ${depth ? "comment-reply" : ""}`}>
      ...
      {panel === "report" && <div className="content-action-overlay" role="dialog" aria-modal="true" aria-label="Report comment"><form className="content-action-card glass compact-content-card" onSubmit={report}><button className="content-action-close" type="button" onClick={() => setPanel(null)} aria-label="Close"><X size={16} /></button><Flag size={18} /><span className="section-label">REPORT COMMENT</span><h3>What’s going on?</h3><select name="reason" defaultValue="spam"><option value="illegal">Illegal content</option><option value="spam">Spam or manipulation</option><option value="harassment">Harassment</option><option value="mature_unmarked">Unmarked mature content</option><option value="other">Something else</option></select><textarea name="details" maxLength={2000} rows={3} placeholder="Optional details" /><button disabled={actionBusy}>{actionBusy ? "Sending…" : "Send report"}</button>{actionMessage && <p role="status"><Check size={14} />{actionMessage}</p>}</form></div>}
      {panel === "edit" && <div className="content-action-overlay" role="dialog" aria-modal="true" aria-label="Edit comment"><form className="content-action-card glass compact-content-card" onSubmit={edit}><button className="content-action-close" type="button" onClick={() => setPanel(null)} aria-label="Close"><X size={16} /></button><Pencil size={18} /><span className="section-label">EDIT COMMENT</span><h3>Refine your comment.</h3><textarea name="body" required maxLength={4000} rows={5} defaultValue={body} /><button disabled={actionBusy}>{actionBusy ? "Saving…" : "Save changes"}</button>{actionMessage && <p role="status">{actionMessage}</p>}</form></div>}
      {panel === "delete" && <div className="content-action-overlay" role="dialog" aria-modal="true" aria-label="Delete comment"><div className="content-action-card glass compact-content-card delete-confirmation"><Trash2 size={20} /><span className="section-label">DELETE COMMENT</span><h3>Delete this comment forever?</h3><p>Any replies underneath it will also be permanently removed.</p><div><button type="button" onClick={() => setPanel(null)}>Keep comment</button><button type="button" className="danger-action" onClick={deleteComment} disabled={actionBusy}>{actionBusy ? "Deleting…" : "Delete forever"}</button></div>{actionMessage && <small role="status">{actionMessage}</small>}</div></div>}
    </article>
  );
}
