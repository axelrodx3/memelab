...
export default function PostCard({ post, viewerId, showMature = false, detail = false }) {
  ...
  return (
    <article className={`community-post glass ${detail ? "post-detail-card" : ""}`}>
      ...
      {panel === "report" && <div className="content-action-overlay" role="dialog" aria-modal="true" aria-label="Report post"><form className="content-action-card glass" onSubmit={report}><button className="content-action-close" type="button" onClick={() => setPanel(null)} aria-label="Close"><X size={16} /></button><Flag size={19} /><span className="section-label">REPORT POST</span><h3>What’s going on?</h3><select name="reason" defaultValue="spam"><option value="illegal">Illegal content</option><option value="spam">Spam or manipulation</option><option value="harassment">Harassment</option><option value="mature_unmarked">Unmarked mature content</option><option value="other">Something else</option></select><textarea name="details" maxLength={2000} rows={3} placeholder="Optional details" /><button disabled={actionBusy}>{actionBusy ? "Sending…" : "Send report"}</button>{actionMessage && <p role="status"><Check size={14} />{actionMessage}</p>}</form></div>}
      {panel === "edit" && <div className="content-action-overlay" role="dialog" aria-modal="true" aria-label="Edit post"><form className="content-action-card glass content-edit-card" onSubmit={edit}><button className="content-action-close" type="button" onClick={() => setPanel(null)} aria-label="Close"><X size={16} /></button><Pencil size={19} /><span className="section-label">EDIT POST</span><h3>Refine your post.</h3><label>Title<input name="title" required maxLength={140} defaultValue={content.title} /></label><label>Caption<textarea name="caption" maxLength={2000} rows={4} defaultValue={content.caption} /></label><label className="content-check"><input name="mature" type="checkbox" defaultChecked={content.isMature} /> Mark as mature</label><button disabled={actionBusy}>{actionBusy ? "Saving…" : "Save changes"}</button>{actionMessage && <p role="status">{actionMessage}</p>}</form></div>}
      {panel === "delete" && <div className="content-action-overlay" role="dialog" aria-modal="true" aria-label="Delete post"><div className="content-action-card glass delete-confirmation"><Trash2 size={20} /><span className="section-label">DELETE POST</span><h3>Delete this post forever?</h3><p>Its image, comments, votes and activity will be permanently removed. This cannot be undone.</p><div><button type="button" onClick={() => setPanel(null)}>Keep post</button><button type="button" className="danger-action" onClick={deletePost} disabled={actionBusy}>{actionBusy ? "Deleting…" : "Delete forever"}</button></div>{actionMessage && <small role="status">{actionMessage}</small>}</div></div>}
    </article>
  );
}
