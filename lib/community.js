...
import { getProfileRelationship } from "./social";
...
export async function getCommunityPosts(sort = "hot", viewerId = null) {
  ...
  let query = supabase
    ...
    .select(`
      ...
      downvotes_count,comments_count,created_at,edited_at,source_label,
...
export async function getDiscussions(channel = "general", sort = "new", viewerId = null) {
  ...
  let query = supabase
    ...
    .select(`
      id,title,caption,image_url,is_mature,vote_score,upvotes_count,
      downvotes_count,comments_count,created_at,edited_at,source_label,post_kind,channel_slug,
      author:profiles!posts_author_id_fkey(id,username,display_name,avatar_url,karma)
    `)
    .eq("status", "active")
...
export async function getPostComments(postId, viewerId = null) {
  ...
  const { data: comments } = await supabase
    .from("comments")
    .select(`
      id,body,score,parent_id,created_at,edited_at,
...
export const getPublicProfile = cache(async (username, viewerId = null) => {
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL) return null;
  const supabase = await createClient();
  const { data: profile } = await supabase
    .from("profiles")
    .select("id,username,display_name,avatar_url,banner_url,bio,karma,friend_count,created_at,profile_visibility,show_activity,account_status")
    .ilike("username", username)
    .maybeSingle();
  if (!profile || profile.account_status !== "active") return null;

  const relationship = await getProfileRelationship(viewerId, profile.id);

  if (profile.profile_visibility !== "public") {
    return { ...profile, relationship, isPrivate: true, posts: [], comments: [], favorites: [] };
  }
  ...
  return { ...profile, relationship, isPrivate: false, posts, comments, favorites };
});
