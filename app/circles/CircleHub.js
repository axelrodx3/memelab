...
import { CircleAvatar } from "./CircleIdentity";
...
export default function CircleHub({ circles, invites, basePath = "/community/circles" }) {
  ...
  const respond = async (inviteId, action) => {
    ...
    try {
      ...
      if (action === "accept") return window.location.assign(`${basePath}/${result.slug || invites.find((invite) => invite.id === inviteId)?.circle?.slug}`);
      router.refresh();
    } catch (error) {
      ...
      <div className="circles-hub-layout">
        <section>
          ...
          <div className="circle-card-grid">
            {circles.map((circle) => <Link className="circle-card glass" href={`${basePath}/${circle.slug}`} key={circle.id}><div className="circle-card-banner">{circle.banner_url && <Image src={circle.banner_url} alt="" fill sizes="(max-width: 760px) 100vw, 440px" />}</div><div className="circle-card-body"><div className="circle-card-topline"><CircleAvatar circle={circle} size={42} className="circle-card-avatar" /><span className="circle-card-lock"><LockKeyhole size={14} /> Private Circle</span></div><h3>{circle.name}</h3><p>{circle.description || "A private MemeLab space."}</p><footer><span><Users size={14} /> {circle.member_count} {circle.member_count === 1 ? "member" : "members"}</span><strong>{circle.membership.role}</strong></footer></div></Link>)}
