...
export async function getCirclesHome(viewerId) {
  ...
  const [membershipResult, inviteResult] = await Promise.all([
    admin.from("circle_members").select(`${memberFields},circle:circles!circle_members_circle_id_fkey(id,slug,name,description,avatar_url,banner_url,member_count,owner_id,created_at,updated_at)`).eq("user_id", viewerId).eq("status", "active").order("updated_at", { ascending: false }),
    admin.from("circle_invites").select("id,circle_id,invited_user_id,invited_by_id,status,created_at,responded_at,circle:circles!circle_invites_circle_id_fkey(id,slug,name,description,avatar_url,banner_url,member_count),inviter:profiles!circle_invites_invited_by_id_fkey(id,username,display_name,avatar_url)").eq("invited_user_id", viewerId).eq("status", "pending").order("created_at", { ascending: false })
  ]);
