...
export default async function CreateRoomPostPage({ searchParams }) {
  ...
  return <main className="create-post-page"><div className="ambient ambient-one" /><SiteHeader /><section className="discussion-create-shell shell"><header><span className="section-label">START A ROOM POST</span><h1>Bring it to<br /><span>the community.</span></h1><p>Choose the right room, make the topic clear, and invite a real conversation.</p></header><DiscussionForm viewer={viewer} initialChannel={channel} /></section></main>;
}
