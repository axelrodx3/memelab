...
export default function CircleRoom({ circle, membership, members, bannedMembers = [], posts, viewer, basePath = "/community/circles" }) {
  ...
  const focusMembers = () => document.getElementById("circle-members")?.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  return <section className="circle-room-shell shell"><header className="circle-room-hero glass"><CircleCover circle={circle} /><div className="circle-room-identity"><CircleAvatar circle={circle} size={82} /><div className="circle-room-copy"><span className="section-label">PRIVATE MEMELAB CIRCLE</span><h1>{circle.name}</h1><p>{circle.description || "A private space for the people inside it."}</p></div><div className="circle-room-meta"><span><Users size={15} /> {circle.member_count} members</span><RoleChip role={membership.role} /></div>{isOwner && <div className="circle-owner-controls"><button type="button" className="circle-customize-button" onClick={() => setIdentityOpen(true)}><Pencil size={14} /> Customize</button><button type="button" className="circle-members-button" onClick={focusMembers}><Users size={14} /> Members</button><details className="circle-owner-menu"><summary aria-label="Circle owner options"><MoreHorizontal size={17} /></summary><div><button type="button" onClick={() => setDeleteOpen(true)}><Trash2 size={14} /> Delete Circle</button></div></details></div>}</div></header>{message && <p className="social-message" role="status">{message}</p>}
